...
class MemTest86Plus(GenericUpdater):
    ...
    def check_archive_integrity(self) -> bool | int | None:
        latest_version = self._get_latest_version()
        if not isinstance(latest_version, list):
            self.logging_callback("check_archive_integrity: Could not determine latest version.")
            return -1

        new_file_path = self._get_complete_normalized_file_path(absolute=True)
        if not new_file_path:
            self.logging_callback("check_archive_integrity: Could not resolve normalized file path.")
            return -1

        if not isinstance(new_file_path, Path):
            new_file_path = Path(new_file_path)

        archive_path = new_file_path.parent / f"mt86plus_{self._version_to_str(latest_version)}_x86_64.iso.zip"

        if not archive_path.exists():
            self.logging_callback(f"check_archive_integrity: Zip file does not exist: {archive_path}")
            return None
        ...
        sha256_checksum = (
            parse_hash(sha256_checksums_str, [zip_filename], 0, self.logging_callback)
            if sha256_checksums_str
            else None
        )

        if sha256_checksum is None:
            self.logging_callback(f"check_archive_integrity: No SHA256 checksum available for {zip_filename}.")
            return -1

        hash_ok = sha256_hash_check(
            archive_path,
            sha256_checksum,
        ...
        if hash_ok is False:
            self.logging_callback("check_archive_integrity: Zip archive hash check failed.")
            return False

        return True
    
    def check_iso_integrity(self) -> bool | int | None:
        new_file_path = self._get_complete_normalized_file_path(absolute=True)

        if not new_file_path:
            self.logging_callback("check_iso_integrity: Could not resolve normalized file path.")
            return -1
        ...
        if not new_file_path.exists():
            self.logging_callback(f"check_iso_integrity: ISO file does not exist: {new_file_path}")
            return None

        return True

    def check_integrity(self) -> bool | int | None:
        archive_result = self.check_archive_integrity()

        if archive_result in (None, -1, False):
            return archive_result

        iso_result = self.check_iso_integrity()

        if iso_result in (None, -1, False):
            return iso_result

        self.logging_callback("check_integrity: Both zip and ISO file passed integrity check.")
        return True
    ...
    def install_latest_version(self) -> bool | None:
        ...
        integrity = self.check_archive_integrity()
        if integrity == -1:
        ...
        with zipfile.ZipFile(archive_path, "r") as zf:
            iso_member = next((m for m in zf.namelist() if m.endswith(".iso")), None)

            if not iso_member:
                self.logging_callback("ERROR: No .iso file found in archive.")
                return None

            version_str = self._version_to_str(latest_version)

            target_iso = new_file_path.parent / f"{new_file_path.stem.capitalize()}-{version_str}.iso"

            with zf.open(iso_member) as src, open(target_iso, "wb") as dst:
                dst.write(src.read())

        iso = target_iso

        self.logging_callback(f"Extracted ISO directly as: {iso}")
