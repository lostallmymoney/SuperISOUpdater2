...
def robust_download(url: str, local_file, logging_callback, method: str = "GET", retries: int = 4, delay: float = 3.0, chunk_size: int = 1048576, redirects=True, expected_size: Optional[int] = None, **kwargs) -> bool:
    ...
    infinite_retry_on_resume = False
            ...
            with requests.request(method, url, stream=True, headers=headers, timeout=5, allow_redirects=redirects, **kwargs_no_headers) as r:
                ...
                if not download_started:
                    if resume_byte_pos > 0 and r.status_code == 200:
                        ...
                        report(f"[robust_download] Server responded with 200 OK instead of 206 Partial Content for resume request. Mirror does not support resume. ({consecutive_200_on_resume}/6)")
                        if consecutive_200_on_resume >= 6:
                            report(f"[robust_download] Received HTTP 200 on resume 6 times in a row. Deleting .part file and restarting from scratch.")
                            try:
                            ...
                            time.sleep(delay)
                            continue
                        # If we ever get a 200 on resume, allow infinite retries from now on
                        infinite_retry_on_resume = True
                        time.sleep(delay)
                        continue
                    elif resume_byte_pos > 0 and r.status_code == 206:
                        # Got a 206, reset the 200-on-resume counter and allow infinite retries
                        consecutive_200_on_resume = 0
                        infinite_retry_on_resume = True
                    else:
                        consecutive_200_on_resume = 0
                    ...
                    if r.status_code in RETRYABLE_STATUSES or r.status_code in PERMANENT_FAILURE_STATUSES or (r.status_code != 200 and r.status_code != 206):
                        if not infinite_retry_on_resume:
                            attempt += 1
